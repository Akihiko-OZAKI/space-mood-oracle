# 🚀 最終ステップ - Renderでデプロイ

## ✅ 完了したこと

- ✅ TiDB Cloud データベース作成
- ✅ テーブル作成
- ✅ 接続文字列取得
- ✅ GitHubリポジトリ作成
- ✅ プロジェクトをプッシュ

---

## 🎯 最終ステップ: Renderでデプロイ

### ステップ1: RenderでWebサービスを作成

1. **Renderダッシュボードにアクセス**
   - https://render.com/ にアクセス
   - ログイン

2. **「New +」→「Web Service」を選択**

3. **リポジトリを選択**
   - GitHubリポジトリから選択
   - `Akihiko-OZAKI/space-mood-oracle` を選択

### ステップ2: サービス設定

以下の設定を入力:

| 項目 | 値 |
|------|-----|
| **Name** | `space-mood-oracle` |
| **Region** | `Oregon (US West)` など |
| **Branch** | `main` |
| **Root Directory** | （空欄） |
| **Runtime** | `Node` |
| **Build Command** | `pnpm install && pnpm build` |
| **Start Command** | `pnpm start` |
| **Instance Type** | `Free` |

**注意:**
- **Root Directory**: 空欄（プロジェクトルートがリポジトリルートなので）

### ステップ3: 環境変数を設定

「**Environment**」セクションで、以下の環境変数を追加:

#### 環境変数1: DATABASE_URL

```
Key: DATABASE_URL
Value: mysql://YOUR_USERNAME:YOUR_PASSWORD@YOUR_HOST:4000/space_mood_oracle?ssl-mode=REQUIRED
```

#### 環境変数2: JWT_SECRET

```
Key: JWT_SECRET
Value: your-random-secret-key-change-this-in-production
```

**注意**: ランダムな文字列に変更してください（例: `openssl rand -hex 32` で生成）

#### 環境変数3: NODE_ENV

```
Key: NODE_ENV
Value: production
```

#### 環境変数4: PORT

```
Key: PORT
Value: 10000
```

### ステップ4: デプロイ開始

1. すべての設定を確認
2. **「Create Web Service」** をクリック
3. デプロイが自動的に開始されます ✅

---

## ⏳ デプロイ待機（約5-10分）

### デプロイプロセス

1. **依存関係のインストール**（約2-3分）
   - `pnpm install` が実行される

2. **ビルド**（約3-5分）
   - `pnpm build` が実行される
   - ReactとExpressがビルドされる

3. **起動**（約1分）
   - `pnpm start` が実行される
   - アプリが起動する

### ログの確認

- デプロイ中はログがリアルタイムで表示されます
- エラーがあればログに表示されます

---

## ✅ デプロイ完了の確認

### 1. ダッシュボードで確認

- 「**Live**」と表示されれば成功 ✅
- URLが表示されます（例: `https://space-mood-oracle.onrender.com`）

### 2. アプリにアクセス

- URLをクリックしてアプリにアクセス
- 正常に表示されれば成功 ✅

---

## 🔧 トラブルシューティング

### ビルドエラー

**エラー**: "Build failed"

**解決策:**
- ログを確認
- `package.json` のビルドスクリプトを確認
- 依存関係を確認

### 起動エラー

**エラー**: "Application failed to start"

**解決策:**
- 環境変数を確認
- データベース接続を確認
- ポート設定を確認（`PORT=10000`）

### データベース接続エラー

**エラー**: "Cannot connect to database"

**解決策:**
- `DATABASE_URL` を確認
- TiDB Cloudの接続文字列を確認
- SSL設定を確認（`?ssl-mode=REQUIRED`）

---

## 📝 まとめ

### セットアップ完了

1. ✅ TiDB Cloud データベース作成
2. ✅ テーブル作成
3. ✅ 接続文字列取得
4. ✅ GitHubにプッシュ
5. ✅ Renderでデプロイ

### コスト

**合計: $0/月（完全無料）** ✅

- TiDB Cloud: 無料（Developer Tier）
- Render: 無料（スリープあり）

---

## 🎉 完了！

これで本番環境のセットアップが完了しました！

アプリが正常に動作していることを確認してください。

---

**Renderでデプロイを開始しましょう！** 🚀


